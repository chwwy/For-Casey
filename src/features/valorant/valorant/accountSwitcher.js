...
import {dbGetAccount, dbSaveAccount, dbDeleteAccount} from "../valorantDb.js";
...
export const saveUserJson = (id, json) => {
    let region = null;
    if (json && json.accounts && json.currentAccount) {
        const activeAcc = json.accounts[json.currentAccount - 1];
        if (activeAcc) region = activeAcc.region;
    }
    dbSaveAccount(id, json, region);
}
...
export const deleteUser = (id, accountNumber) => {
    ...
    if(userJson.accounts.length === 0) dbDeleteAccount(id);
