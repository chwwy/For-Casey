...
export const init = async (discordClient) => {
...
export const handleMessage = async (message) => {
    try {
        ...
        const adminCmds = ["!deploy", "!undeploy", "!config", "!message", "!status", "!forcealerts", "!stop", "!update"];
        const firstWord = content.split(' ')[0];
        if (!adminCmds.includes(firstWord)) return false;
        ...
        try {
            ...
            switch (selectType) {
                ...
                case "select-skin-with-level": {
                    ...
                    const skinUuidForOptions = skinUuid;
                        ...
                        if (level.streamedVideo) {
                            ...
                            levelSelector.addOptions(
                                new StringSelectMenuOptionBuilder()
                                    .setLabel(`${skinName}`)
                                    .setValue(`levels/${level.uuid}/${skinUuidForOptions}`));
                            optionsCount++;
                        }
                    }

                    for (let i = 1; i < skin.chromas.length; i++) { // this change skips the default version of the skin because it is the same as level 1
                        ...
                        levelSelector.addOptions(
                            new StringSelectMenuOptionBuilder()
                                .setLabel(`${chromaName}`)
                                .setValue(`chromas/${chromas.uuid}/${skinUuidForOptions}`));
                        optionsCount++;
                    }

                    if (optionsCount === 0) {
                        return await interaction.reply({
                            content: "This skin does not have any additional levels or chromas with showcase videos.",
                            ephemeral: true
                        });
                    }
                ...
                case "get-level-video": {
                    ...
                    if (config.videoViewerWithSite || config.imageViewerWithSite) {
                        ...
                        await interaction.editReply({
                            ...
                            files: [{
                                attachment: link,
                                name: `${name.replace(/[^a-zA-Z0-9]/g, "_")}.${skin[0].streamedVideo ? "mp4" : "png"}`
                            }]
                        });
                    }
                }
        ...
        try {
            if (interaction.customId.startsWith("gotopage")) {
                ...
                async function clwpage() {
                    ...
                    const skinsResponse = await getSkins(user);
                    if (!skinsResponse.skinsResponse) return await interaction.reply(authFailureMessage(interaction, skinsResponse, s(interaction).error.AUTH_ERROR_COLLECTION, userId !== interaction.user.id));
...
export const startBot = () => {
    console.log("startBot called: login managed by main process");
}
