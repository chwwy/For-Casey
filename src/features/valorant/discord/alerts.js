...
import {dbAddAlert, dbRemoveAlert, dbGetAlerts} from "../valorantDb.js";
...
export const addAlert = (id, alert) => {
    dbAddAlert(id, alert.uuid, alert.channel_id);
}

export const alertsForUser = (id, account=null) => {
    return dbGetAlerts(id);
}
...
export const removeAlert = (id, uuid) => {
    const beforeCount = dbGetAlerts(id).length;
    dbRemoveAlert(id, uuid);
    const afterCount = dbGetAlerts(id).length;
